...
use Bio::Range;
use Bio::SeqIO;
use RNAseq_pipeline2 qw(get_fh parse_gff_line get_feature_overlap get_feat_mappable_length_sub);
use RNAseq_pipeline_settings ('read_config_file','get_dbh','read_file_list',
			      'get_gene_from_junc_sub');
...
$feat_map_table='feature_mappable_lengths';

# Connect to the database
my $dbh=get_dbh();

# Get some usefull subs
*junc2gene=get_gene_from_junc_sub($dbh);
*mappable_length=get_feat_mappable_length_sub($dbh,
					      $readlength,
					      $feat_map_table);
...
foreach my $lane (keys %lanes) {
    ...
    my $outfile=$genomedir.'/'.$lane.'.'.$type.'.gene.rpmkm.txt.gz';
    process_exons(\%genes,
		  \%gene_coverage,
		  \%gene_juncs_coverage,
		  $outfile,
...
sub get_projected_length {
	...
	while (my $line=<$projfh>){
	    ...
	    my $proj_id=join('_',
			     ...
			     $line{'start'},
			     $line{'end'});
	    my $length=mappable_length($proj_id);
	    $lengths->{$gene}+=$length;
	}
	close($projfh);
    } else {
...
sub process_exons {
	...
	if ($exoncoverage->{$gene} > $rpkm_ex) {
	    $rpkm_ex=$exoncoverage->{$gene};
	}

	# add coverage from junctions
	if ($junccoverage->{$gene} > $rpkm_jun) {
	    $rpkm_jun=$junccoverage->{$gene};
	}

	# Normalize by read number
	my $length=$gene_lengths->{$gene};
	my $rpkm=$rpkm_ex + $rpkm_jun;
	if ($length) {
