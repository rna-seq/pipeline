...
sub get_mergers {
	...
	my $splitmappingfh=get_mapping_fh($lane,
					   'split');
	my $recursivemappingfh=get_mapping_fh($lane,
					      'recursive');
...
#	    print STDERR $read_id_split,"\n";
#	    print STDERR $read_id_rec,"\n";
		...
		if ($juncmatches=~/^0(:0)*:0$/) {
		    ...
		    # Set the maximum number of allowed mismatches to the ones
		    # found in the genome
