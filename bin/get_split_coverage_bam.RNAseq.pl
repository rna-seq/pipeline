...
my $projected_splits_fn=$clusterdir.'/'.$project.'.unique.junctions.cluster.gtf.gz';
unless (-r $projected_splits_fn) {
    die "Cannot find the unique clustering of the junction maps at $projected_splits_fn\n";
}
...
foreach my $lane (keys %lanes) {
    my $infilename=$juncdir.'/'.$lane.'.'.$type.'.unique.gtf.gz';
    ...
    my $outfile=$juncdir.'/'.$lane.'.'.$type.'.unique.breakdown.gtf.gz';
