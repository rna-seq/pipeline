...
sub get_mergers {
	...
	my $lastrecurs=<$recursivemappingfh>;
	my %lastrecurs=%{parse_gem_line($lastrecurs)};
	my ($genome);
	my $genomelineno=0;
	my $junctionlineno=0;
	while ($genome=<$genomemappingfh>) {
	    ...
	    unless ($linegenome{'seq'} eq $linejunc{'seq'}) {
		print STDERR join("\t",
				  $genomelineno,
				  $read_id,
				  $junctionlineno,
				  $linejunc{'id'}),"\n";
		warn "Incorrect read, are you sure files are sorted\n";
	    }
