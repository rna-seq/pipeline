...
foreach my $lane (keys %lanes) {
    ...
    my $infilename=$junctiondir.'/'.$lane.'.'.$type.'.unique.overlap.total';
...
# Extract from the annotation the number of
# features in the file and use this as the limit to the y axis
my $no_features=get_junction_no();
...
sub plot_expressed {
    ...
    $r_string.="xlab=\"Lanes\",ylab=\"$graph detected\",log=\"y\",";
    $r_string.="col=cols,ylim=c(1,$no_features))\n";
