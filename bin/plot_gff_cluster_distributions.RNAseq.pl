...
# Get the outfile
my $outfile=$options{'PREFIX'}.'_initial_clusters';
...
foreach my $type (keys %files) {
    ...
    my $infilename=$projectid.'.unique.'.$type.'.cluster.gtf.gz';
...
sub build_graph {
    ...
    $r_string.="main=\"$species $project Clusters of unique maps\",";
    $r_string.="xlab=\"Chromosomes\",ylab=\"Clusters\",legend=T,";
    $r_string.="col=cols)\n";
    $r_string.="dev.off()\n";

    $r_string.="jpeg(\"$graph.jpeg\")\n";
    $r_string.='barplot(stats[,order(as.numeric(colnames(stats)))],names.arg=attributes(stats)$row.names,beside=T,';
    $r_string.="main=\"$species $project Clusters of unique maps\",";
    $r_string.="xlab=\"Chromosomes\",ylab=\"Clusters\",legend=T,";
    $r_string.="col=cols)\n";
