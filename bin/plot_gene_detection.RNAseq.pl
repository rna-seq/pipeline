...
# Objective
# This script will take as an input a feature file, and a list of overlap.total
# files 
# From the input feature file the script will determine the total number of
...
use RNAseq_pipeline2 qw(get_fh get_Average);
...
$annotation=$genomedir."/$prefix.gene.gtf";
...
foreach my $lane (keys %lanes) {
    ...
    my $infilename=$genomedir.'/'.$lane.'.'.$type.'.unique.gtf.overlap.total';
